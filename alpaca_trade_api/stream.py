...
class DataStream:
    def __init__(self, key_id: str, secret_key: str, base_url: URL, raw_data: bool):
        self._key_id = key_id
    ...
    def _cast(self, msg_type, msg):
        ...
        if msg_type == 't':
            return Trade({trade_mapping_v2[k]: v for k, v in msg.items()
                          if k in trade_mapping_v2})
        elif msg_type == 'q':
            return Quote({quote_mapping_v2[k]: v for k, v in msg.items()
                          if k in quote_mapping_v2})
        elif msg_type == 'b':
            return Bar({bar_mapping_v2[k]: v for k, v in msg.items()
    ...
    async def _dispatch(self, msg):
        ...
        if msg_type == 't':
            ...
            handler = self._bar_handlers.get(
                symbol, self._bar_handlers.get('*', None))
            if handler:
                await handler(self._cast(msg_type, msg))
...
class Stream:
    def __init__(self,
        ...
        self._data_ws = DataStream(
            self._key_id, self._secret_key, self._data_url, raw_data)
